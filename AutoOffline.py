...
SENSITIVE_KEYS = {
    "secret", "shared_secret", "totp_secret", "password", "mail_password",
    "license_key", "server_token", "cookie", "golden_key", "recovery_code",
}
ACCOUNT_TYPES = {
...
NOTIFICATION_LABELS = {
    "command_request": "Запросы команд",
    "code_issued": "Успешные выдачи",
    "request_denied": "Ошибки выдачи",
    "new_order": "Новые заказы",
    "order_status": "Статусы заказов",
    "lot_status": "Состояние лотов",
    "denuvo": "Denuvo",
...
LOG_EVENT_META = {
    "START": ("🚀", "ЗАПУСК", "cyan"),
    "ORDER": ("🛒", "ЗАКАЗ", "cyan"),
    "ORDER_REPAIRED": ("🔗", "ЗАКАЗ", "green"),
    "ORDER_UNRESOLVED": ("⚠️", "ЗАКАЗ", "yellow"),
    "ORDER_MATCH_FAILED": ("⚠️", "ЗАКАЗ", "yellow"),
    "ISSUED": ("✅", "ВЫДАЧА", "green"),
    "DENIED": ("⛔", "ОТКАЗ", "yellow"),
    "DENUVO": ("🎮", "DENUVO", "magenta"),
    "LOT": ("🛒", "ЛОТ", "cyan"),
    "REFUND": ("↩️", "ВОЗВРАТ", "yellow"),
    "REFUND_IGNORED": ("↩️", "ВОЗВРАТ", "yellow"),
    "SERVER": ("🌐", "СЕРВЕР", "cyan"),
    "SETTINGS": ("⚙️", "НАСТРОЙКИ", "cyan"),
    "MAINTENANCE": ("🛠", "СЕРВИС", "cyan"),
    "WARNING": ("⚠️", "ВНИМАНИЕ", "yellow"),
    "WARN": ("⚠️", "ВНИМАНИЕ", "yellow"),
    "ERROR": ("❌", "ОШИБКА", "red"),
    "FAILED": ("❌", "ОШИБКА", "red"),
...
LOG_DETAIL_LABELS = {
    ...
    "status": "статус",
    "attempts": "попытки",
    "active_slots": "занято слотов",
    "slot_limit": "лимит слотов",
}
...
def _normalize_title(value: Any) -> str:
    ...
    return re.sub(r"[^0-9a-zа-яё]+", " ", text, flags=re.I).strip()
...
def _default_db() -> dict:
    return {
        ...
        "global": {
            ...
            "notifications": {key: True for key in NOTIFICATION_LABELS},
            "server": {
                "url": DEFAULT_SERVER_URL,
                "token": os.getenv("AUTOOFFLINE_SERVER_TOKEN", "").strip(),
                "license_key": os.getenv("AUTOOFFLINE_LICENSE_KEY", "").strip(),
                "license_type": "manual" if os.getenv("AUTOOFFLINE_LICENSE_KEY", "").strip() else "auto",
                "cardinal_id_hash": "",
                "owner_id": os.getenv("AUTOOFFLINE_OWNER_ID", "").strip(),
                "seller_hash": os.getenv("AUTOOFFLINE_SELLER_HASH", "").strip().lower(),
                "timeout": 15,
                ...
                "installation_secret": "",
                "installation_id": "",
                "registered_build_hash": "",
                "registered_owner_id": "",
                "protocol_version": 1,
                "data_binding_mode": "off",
                "clock_offset": 0,
                "last_register_at": 0,
                "last_verify_at": 0,
                "last_verify_code": "NOT_CONFIGURED",
                "last_status_sync": 0,
                "active_reservations": 0,
                "queue_max_attempts": 9,
                "queue_retry_base": 5,
                "queue_max_wait": 1800,
                "denuvo_hold_seconds": _safe_int(os.getenv("AUTOOFFLINE_DENUVO_HOLD_SECONDS"), 21600, 300, 604800),
                "status_sync_interval": 300,
            },
            "templates": {
                ...
                "denuvo_queued": "🕒 Denuvo-запрос принят. Позиция в очереди: <b>{position}</b>. Повторять команду не нужно.",
                "denuvo_duplicate": "🕒 Этот Denuvo-запрос уже обрабатывается. Позиция: <b>{position}</b>.",
                "steam_queued": "🕒 Запрос Steam Guard поставлен в очередь. Позиция: <b>{position}</b>. Следующий код будет выдан не раньше чем через 15 секунд.",
                "steam_duplicate": "🕒 Ваш запрос Steam Guard уже находится в очереди. Позиция: <b>{position}</b>.",
                "totp_queued": "🕒 Запрос TOTP поставлен в очередь. Позиция: <b>{position}</b>. Новый код будет выдан в следующем 30-секундном окне.",
                "totp_duplicate": "🕒 Такой TOTP-запрос уже находится в очереди. Позиция: <b>{position}</b>.",
                "denied_no_order": "❌ Не найден подходящий оплаченный заказ для этого лота.",
                "denied_limit": "❌ Лимит по заказу исчерпан.",
                "denied_cooldown": "⏳ Повторный запрос будет доступен через {wait}.",
                "unavailable": "❌ Автоматическая выдача временно недоступна. Напишите продавцу.",
            },
        },
        "accounts": [],
        "lots": [],
        ...
        "stats": {
            ...
            "server_errors": 0,
            "queue_enqueued": 0,
            "queue_enqueued_messages": 0,
            "queue_completed": 0,
            "queue_failed": 0,
            "steam_queue_enqueued": 0,
            "steam_queue_completed": 0,
            "steam_queue_failed": 0,
        },
    }
...
def _unlock_password(password: str) -> bool:
    ...
    with _DB_LOCK:
        ...
        iterations = _safe_int(kdf.get("iterations"), PBKDF2_ITERATIONS, 100_000, 2_000_000)
...
def _order_buyer_matches(order: dict, chat_id: Any, buyer_id: str, buyer_nick: str) -> bool:
    ...
    return bool(
        (order_chat and chat and order_chat == chat)
        or (order_buyer and buyer and order_buyer == buyer)
        or (order_nick and nick and order_nick == nick)
    )
...
def _repair_order_lot_link(order: dict, lot: dict, reason: str) -> bool:
    ...
    if changed:
        ...
        _log(
            "ORDER_REPAIRED",
            "Связь заказа с лотом восстановлена",
...
def generate_totp_code(secret_value: str, digits: int = 6, period: int = 30) -> Optional[str]:
    try:
        ...
        offset = digest[-1] & 0x0F
        value = (struct.unpack(">I", digest[offset:offset + 4])[0] & 0x7FFFFFFF) % (10 ** int(digits))
...
def fetch_rockstar_code(account: dict) -> Tuple[Optional[str], str]:
    ...
    code_regex = str(data.get("code_regex") or r"\b([A-Z0-9]{6,8})\b")
...
def _extract_order_info(event: Any) -> dict:
    ...
    info = {
        "order_id": "",
        "chat_id": str(_event_chat_id(event) or _field(order, "chat_id") or ""),
        "buyer_id": "",
        "buyer_nick": "",
        "funpay_lot_id": "",
        "quantity": 1,
    ...
    for obj in objects:
        if not info["order_id"]:
            for key in ("order_id", "id"):
                ...
                if value not in (None, "") and obj is not _field(order, "lot"):
                    ...
                    if re.fullmatch(r"[A-Za-z0-9-]{5,}", candidate):
        ...
        if not info["funpay_lot_id"]:
            for key in ("html", "link", "url", "public_link", "private_link"):
                ...
                match = re.search(r"(?:offer(?:Edit)?\\?(?:id|offer)=|data-offer-id=[\\\"']?)(\\d+)", raw, re.I)
                if match:
...
LOT_TITLE_STOPWORDS = {
    "покупатель", "оплатил", "оплатила", "заказ", "новый", "steam", "оффлайн",
    "активации", "активация", "игр", "игра", "товар", "код", "подтвердить",
    "выполнение", "order", "paid", "buyer", "offline", "activation", "games",
}
...
def _record_order(info: dict) -> Optional[dict]:
    ...
    if not order_id:
        raw = "|".join([
            str(info.get("chat_id") or ""),
            str(info.get("buyer_id") or ""),
            str(info.get("buyer_nick") or ""),
            str(info.get("funpay_lot_id") or ""),
    ...
    if existing is None:
        existing = {
            ...
            "buyer_id": str(info.get("buyer_id") or info.get("chat_id") or ""),
            "buyer_nick": str(info.get("buyer_nick") or ""),
            "funpay_lot_id": str(info.get("funpay_lot_id") or (lot or {}).get("funpay_lot_id") or ""),
            "lot_id": str((lot or {}).get("id") or ""),
            "title": str(info.get("title") or "")[:500],
            "quantity": max(1, _safe_int(info.get("quantity"), 1, 1, 100000)),
            "status": "paid",
            "created_at": _now(),
            "updated_at": _now(),
            "allocations": {},
            "issued": 0,
            "unresolved": lot is None,
            "new_order_notified": False,
        }
    ...
    if lot is None:
        if created:
            _log(
                "ORDER_UNRESOLVED",
                "Заказ сохранён без привязки к лоту. Привязка будет восстановлена по команде покупателя",
                order_id=order_id,
                funpay_lot_id=info.get("funpay_lot_id"),
                buyer=existing.get("buyer_nick") or existing.get("buyer_id"),
        ...
        if should_announce:
            _notify(
                "new_order",
                f"⚠️ <b>Заказ получен, но лот пока не сопоставлен</b>\n"
                f"Покупатель: <code>{escape(str(existing.get('buyer_nick') or existing.get('buyer_id')))}</code>\n"
    ...
    if should_announce:
        _notify(
            ...
            f"Тип: <b>{escape(ACCOUNT_TYPES.get(_canonical_account_type(lot.get('account_type')), str(lot.get('account_type'))))}</b>\n"
...
def _mark_order_from_system_message(event: Any, text: str) -> None:
    ...
    if not any(token in low for token in ("оплатил заказ", "заказ оплачен", "paid the order", "order paid")):
        return
    oid_match = re.search(r"(?:заказ|order)\s*#?\s*([A-Za-z0-9\-]{5,})", text, re.I)
    qty_match = re.search(r"(\d{1,5})\s*(?:шт|коп|copies|items?)", text, re.I)
    info = {
...
def _mark_refund_from_system_message(event: Any, text: str) -> bool:
    ...
    refund_tokens = (
        "вернул деньги", "возврат средств", "заказ возвращ", "возврат по заказу",
        "refund", "refunded", "money was returned", "order was refunded",
    )
    if not any(token in low for token in refund_tokens):
        return False
    db = _db_get()
    oid_match = re.search(r"(?:заказ|order)\s*#?\s*([A-Za-z0-9\-]{5,})", str(text), re.I)
    order = None
    if oid_match:
    ...
    if order is None:
        ...
        buyer_id, buyer_nick = _extract_buyer(event)
        candidates = [
            row for row in db.get("orders", [])
            if str(row.get("status") or "") in {"paid", "active"}
            and (
                str(row.get("chat_id") or "") == chat_id
                or str(row.get("buyer_id") or "") == str(buyer_id or "")
                or (buyer_nick and str(row.get("buyer_nick") or "").casefold() == str(buyer_nick).casefold())
            )
        ]
    ...
    recent = [
        {
            ...
            "funpay_lot_id": row.get("funpay_lot_id"),
            "buyer": row.get("buyer_nick") or row.get("buyer_id"),
            "status": row.get("status"),
        }
        for row in _db_get().get("orders", [])[-8:]
    ]
    _log(
        "ORDER_MATCH_FAILED",
        "Не найден оплаченный заказ для команды",
        lot_id=lot.get("id"),
        funpay_lot_id=lot.get("funpay_lot_id"),
        account_type=_canonical_account_type(lot.get("account_type")),
        chat_id=chat_id,
...
DENUVO_RETRYABLE_CODES = {
    "NETWORK_ERROR", "TIMEOUT", "RATE_LIMITED", "INTERNAL_ERROR", "NO_SAFE_FREE_ACCOUNT",
    "SERVER_UNAVAILABLE", "HTTP_429", "HTTP_500", "HTTP_502", "HTTP_503", "HTTP_504",
    "TIMESTAMP_OUT_OF_RANGE", "database_locked", "server_url_empty", "installation_not_registered",
}
DENUVO_PERMANENT_CODES = {
    "LICENSE_NOT_FOUND", "LICENSE_DISABLED", "LICENSE_EXPIRED", "PLUGIN_UUID_MISMATCH",
    "OWNER_ID_MISMATCH", "SELLER_HASH_MISMATCH", "BUILD_NOT_ALLOWED", "BUILD_HASH_MISMATCH",
    "MACHINE_HASH_MISMATCH", "INSTALLATION_DISABLED", "INSTALLATION_DISABLED_OR_UNKNOWN",
    "INSTALLATION_LIMIT_REACHED", "DATA_FINGERPRINT_MISMATCH", "ORDER_BUYER_MISMATCH",
    "CARDINAL_ID_MISMATCH", "CARDINAL_ID_REQUIRED", "CARDINAL_ALREADY_LICENSED", "AUTO_LICENSE_DISABLED",
}
...
def _check_usage(lot: dict, order: dict, buyer_id: str) -> Tuple[bool, str, dict, int, int]:
    ...
    rec = db.setdefault("usage", {}).setdefault(
        key,
        {"count": 0, "last_ts": 0, "account_counts": {}, "account_windows": {}},
    )
...
def _cardinal_identity_hash() -> str:
    ...
    payload = {
        "plugin_uuid": UUID,
        "owner_id": owner_id,
        "telegram_bot_token_hash": token_hash,
    }
...
def _data_fingerprint(owner_id: Optional[str] = None) -> str:
    ...
    safe_data = {
        "plugin_uuid": UUID,
        "owner_id": owner_id if owner_id is not None else _resolve_owner_id(),
        "seller_hash": str(server.get("seller_hash") or ""),
        "schema_version": DB_VERSION,
        "installation_id": str(server.get("install_id") or ""),
    }
...
def _identity_body(extra: Optional[dict] = None) -> dict:
    ...
    body = {
        ...
        "owner_id": owner_id,
        "seller_hash": str(server.get("seller_hash") or ""),
        "install_id": str(server.get("install_id") or ""),
        "machine_hash": _machine_hash(),
        "build_hash": _plugin_build_hash(),
        "data_fingerprint": _data_fingerprint(owner_id),
        "version": VERSION,
...
def _http_json(method: str, path: str, payload: Optional[dict] = None, headers: Optional[dict] = None) -> Tuple[bool, dict, str, int]:
    ...
    request_headers = {
        ...
        "User-Agent": f"AutoOffline/{VERSION}",
    }
    if raw is not None:
...
def _server_register(force: bool = False) -> Tuple[bool, dict, str]:
    ...
    if (
        not force
        and str(server.get("installation_secret") or "").strip()
        and str(server.get("registered_build_hash") or "") == build_hash
        and str(server.get("registered_owner_id") or "") == owner_id
    ):
...
def _start_auto_registration() -> None:
    ...
    with _AUTO_REGISTER_LOCK:
        if _AUTO_REGISTER_WORKER is not None and _AUTO_REGISTER_WORKER.is_alive():
            return
        _AUTO_REGISTER_WORKER = threading.Thread(
            target=_auto_registration_run,
            name="AutoOfflineAutoLicense",
            daemon=True,
        )
...
def _signed_headers(body: dict) -> dict:
    ...
    return {
        "X-AO-Install-ID": str(server.get("install_id") or ""),
...
def _server_signed_post(path: str, payload: Optional[dict] = None, retry_registration: bool = True) -> Tuple[bool, dict, str]:
    ...
    _db_save()
    registration_codes = {
        "INSTALLATION_DISABLED_OR_UNKNOWN", "SIGNATURE_INVALID", "MACHINE_HASH_MISMATCH",
        "BUILD_HASH_MISMATCH", "OWNER_ID_MISMATCH", "SELLER_HASH_MISMATCH", "INSTALL_ID_MISMATCH",
    }
...
def _server_allocate(lot: dict, order: dict, buyer_id: str, candidates: List[dict]) -> Tuple[Optional[dict], str, dict]:
    ...
    for account in candidates:
        ...
        failure_rate = 0 if failure_rate < 0.000001 else (1 if failure_rate >= 0.999999 else round(failure_rate, 6))
        payload_candidates.append({
            "account_id": str(account.get("id")),
            "account_name": str(account.get("name") or account.get("id") or ""),
            "enabled": bool(account.get("enabled", True) and denuvo.get("enabled", True)),
            "weight": _safe_int(denuvo.get("weight"), 100, 1, 10000),
            "slot_limit": _safe_int(denuvo.get("slot_limit"), 5, 1, 10000),
            "reserve": _safe_int(denuvo.get("reserve"), 0, 0, 10000),
            "active_slots_local": _safe_int(denuvo.get("active_slots"), 0, 0, 10000),
            "failure_rate": f"{failure_rate:.6f}",
            "cooldown_until": _safe_int(denuvo.get("cooldown_until"), 0),
        })
    payload = {
        "game_id": _denuvo_game_id(lot, candidates),
        "order_id": str(order.get("order_id") or ""),
        "buyer_hash": _buyer_hash(buyer_id),
...
def _server_report(account_id: str, game_id: str, success: bool, error_code: str = "") -> Tuple[bool, dict, str]:
    return _server_signed_post("api/v1/denuvo/report", {
...
def _choose_account(lot: dict, order: dict, buyer_id: str, rec: dict) -> Tuple[Optional[dict], str, dict]:
    ...
    if _is_denuvo_lot(lot):
        ...
        _notify(
            "denuvo",
            f"⚠️ <b>Denuvo API не распределил аккаунт</b>\n"
            f"Лот: <code>{escape(str(lot.get('funpay_lot_id')))}</code>\n"
    ...
    with _DB_LOCK:
        ...
        if activate_denuvo:
            allocation.update({
                ...
                "allocation_mode": "server" if server_data.get("reservation_id") else "local",
                "activation_started_at": now,
            })
            denuvo = account.setdefault("denuvo", {})
            denuvo["active_slots"] = min(
                _safe_int(denuvo.get("slot_limit"), 5, 1, 10000),
                _safe_int(denuvo.get("active_slots"), 0) + 1,
            )
...
def _queue_position(job_id: str) -> int:
    active = sorted(
        [x for x in _queue_jobs() if str(x.get("status")) in DENUVO_ACTIVE_QUEUE_STATES],
        key=lambda x: (_safe_int(x.get("next_attempt_at"), 0), _safe_int(x.get("created_at"), 0), str(x.get("id"))),
    )
...
def _queue_enqueue(lot: dict, order: dict, chat_id: Any, buyer_id: str, buyer_nick: str) -> Tuple[dict, bool, int]:
    with _DB_LOCK:
        ...
        for existing in _queue_jobs():
            same_buyer_lot = (
                str(existing.get("lot_id") or "") == str(lot.get("id") or "")
                and str(existing.get("buyer_id") or "") == str(buyer_id or "")
            )
...
def _queue_claim() -> Optional[str]:
    ...
    with _DB_LOCK:
        candidates = [
            x for x in _queue_jobs()
            if str(x.get("status")) in {"queued", "retry", "lifecycle_retry", "holding"}
            and _safe_int(x.get("next_attempt_at"), 0) <= now
        ]
        if not candidates:
            return None
...
def _funpay_plain_text(text: str) -> str:
    ...
    value = re.sub(r"(?i)</?(?:b|strong|code|i|em|u|s|pre)>", "", value)
...
def _delivery_mapping(lot: dict, order: dict, account: dict, buyer_nick: str, code: str, left: int, total: int, allocation_mode: str) -> dict:
    ...
    return {
        ...
        "lot": lot.get("title") or lot.get("funpay_lot_id"),
        "order_id": order.get("order_id"),
        "buyer": buyer_nick,
        "left": left,
        "total": total,
        "quantity": order.get("quantity"),
        "slot_info": f"{denuvo.get('active_slots', 0)}/{denuvo.get('slot_limit', 5)}",
...
def _steam_queue_key(lot: dict, order: dict, buyer_id: str) -> str:
    raw = "|".join([
        str(lot.get("account_type") or "otp"),
        str(lot.get("id")),
...
def _steam_queue_position(job_id: str) -> int:
    ...
    active = sorted(
        [
            x for x in _steam_queue_jobs()
            if str(x.get("status")) in STEAM_QUEUE_ACTIVE_STATES
            and (not lot_id or str(x.get("lot_id")) == lot_id)
        ],
        key=lambda x: (_safe_int(x.get("next_attempt_at"), 0), _safe_int(x.get("created_at"), 0), str(x.get("id"))),
    )
    for index, job in enumerate(active, 1):
...
def _steam_queue_claim() -> Optional[str]:
    ...
    with _DB_LOCK:
        candidates = [
            x for x in _steam_queue_jobs()
            if str(x.get("status")) in {"queued", "retry"}
            and _safe_int(x.get("next_attempt_at"), 0) <= now
        ]
        if not candidates:
            return None
...
def _process_steam_queue_job(job_id: str) -> None:
    ...
    candidates = [
        account for account in _candidate_accounts(lot, order, rec)
        if _canonical_account_type(account.get("type")) == account_type
        and bool((account.get("queue") or {}).get("enabled"))
        and (not pinned_account_id or str(account.get("id")) == pinned_account_id)
    ]
    if not candidates:
    ...
    if wait_seconds > 0:
        reason = (
            "Ожидание следующего 30-секундного TOTP-окна."
            if account_type == "totp"
            else "Ожидание 15-секундного интервала Steam Guard."
        )
    ...
    _log(
        "ISSUED",
        f"{label}-код выдан через очередь",
        order_id=order.get("order_id"),
        lot_id=lot.get("id"),
        account_id=account.get("id"),
        buyer=buyer_nick,
    ...
    _notify(
        "code_issued",
        f"✅ <b>{label} выдан через очередь</b>\n"
        f"Аккаунт: <b>{escape(str(account.get('name')))}</b>\n"
        f"Покупатель: <code>{escape(buyer_nick)}</code>\n"
        f"Осталось: <b>{left}/{total}</b>",
    )
...
def _enqueue_steam_request(
    ...
    label = "TOTP" if account_type == "totp" else "Steam Guard"
    try:
        job, created, position = _steam_queue_enqueue(
            lot, order, chat_id, buyer_id, buyer_nick, account_id=account_id
        )
    except Exception as e:
    ...
    if should_notify:
        ...
        _funpay_send(
            chat_id,
            _render(queue_text, {"position": position or 1, "queue_id": job.get("id")}),
        )
    return True
...
def _reuse_denuvo_activation(
    ...
    if account is None or not bool(account.get("enabled", True)):
        ...
        _log(
            "ERROR",
            "Активная Denuvo-активация ссылается на недоступный аккаунт",
            order_id=order.get("order_id"),
    ...
    if account_type in {"steam_sda", "totp"} and bool((account.get("queue") or {}).get("enabled")):
        _log(
            "DENUVO",
            "Активация уже существует. Код направлен в обычную очередь",
            order_id=order.get("order_id"),
            lot_id=lot.get("id"),
            account_id=account.get("id"),
            buyer=buyer_nick,
        )
        return _enqueue_steam_request(
            cardinal,
            chat_id,
            lot,
            order,
    ...
    if provider_status != "ok" or not code:
        ...
        _log(
            "ERROR",
            "Не удалось получить код для существующей Denuvo-активации",
            order_id=order.get("order_id"),
            account_id=account.get("id"),
    ...
    if not sent:
        _log(
            "ERROR",
            "Не удалось отправить повторный код покупателю",
            error=send_error,
            order_id=order.get("order_id"),
            account_id=account.get("id"),
        )
    ...
    _log(
        "DENUVO",
        "Код выдан без создания нового Denuvo-слота",
        order_id=order.get("order_id"),
        lot_id=lot.get("id"),
        account_id=account.get("id"),
        buyer=buyer_nick,
        ...
        slot_limit=_safe_int(denuvo.get("slot_limit"), 5),
    )
    _notify(
        "code_issued",
        f"🔐 <b>Steam Guard выдан по активной Denuvo-активации</b>\n"
        f"Покупатель: <code>{escape(buyer_nick)}</code>\n"
        f"Лот: <b>{escape(str(lot.get('title') or lot.get('funpay_lot_id')))}</b>\n"
        f"Аккаунт: <b>{escape(str(account.get('name')))}</b>\n"
        f"Осталось кодов: <b>{left}/{total}</b>\n"
        f"Denuvo-слоты: <b>{_safe_int(denuvo.get('active_slots'), 0)}/{_safe_int(denuvo.get('slot_limit'), 5)}</b>",
    )
    return True
    ...
    for (lot_id, buyer_id), rows in groups.items():
        ...
        for duplicate in rows[1:]:
            ...
            if not ok:
                _log(
                    "WARNING",
                    "Не удалось освободить повторную Denuvo-активацию",
                    lot_id=lot_id,
                    buyer_id=buyer_id,
            ...
            _log(
                "DENUVO",
                "Повторная Denuvo-активация автоматически освобождена",
                lot_id=lot_id,
                buyer_id=buyer_id,
                kept_reservation_id=keep.get("reservation_id"),
...
def _locked_text() -> str:
    ...
    return (
        "🔐 <b>Хранилище AutoOffline недоступно</b>\n\n"
        f"Состояние: <b>{escape(status)}</b>\n"
        f"Режим: <code>{escape(mode)}</code>\n\n"
        "База в этой версии открывается локальным ключом автоматически. Если сейчас она недоступна, "
        "значит повреждён файл базы или ключ. Проверьте хранилище либо восстановите бэкап."
    )
...
def _home_text() -> str:
    return (
        ...
        f"👤 Автор: <a href=\"{CREATOR_URL}\">{escape(CREDITS)}</a>\n\n"
        "Выберите раздел:"
    )
...
def _settings_text() -> str:
    ...
    return (
        "<b>⚙️ Панель настроек AutoOffline</b>\n\n"
        f"• Аккаунты: <b>{enabled_accounts}/{len(db.get('accounts', []))}</b>\n"
        f"• Лоты: <b>{enabled_lots}/{len(db.get('lots', []))}</b>\n"
        f"• Заказы в базе: <b>{len(db.get('orders', []))}</b>\n\n"
        "Выберите категорию:"
    )
...
def _lots_text() -> str:
    ...
    for index, lot in enumerate(lots, 1):
        ...
        lines.append(
            f"{index}. {_lot_state_icons(lot)} <b>{escape(str(lot.get('title') or lot.get('funpay_lot_id')))}</b> · "
            f"LOT <code>{escape(str(lot.get('funpay_lot_id')))}</code> · "
            f"<code>{escape(str(lot.get('command')))}</code>{denuvo}"
        )
    return "\n".join(lines)
...
def _lot_text(lot: dict) -> str:
    ...
    return (
        f"<b>🛒 {escape(str(lot.get('title') or lot.get('funpay_lot_id')))}</b>\n\n"
        f"• LOT ID: <code>{escape(str(lot.get('funpay_lot_id')))}</code>\n"
        f"• Внутренний ID: <code>{escape(str(lot.get('id')))}</code>\n"
        f"• Тип: <b>{escape(ACCOUNT_TYPES.get(str(lot.get('account_type')), str(lot.get('account_type'))))}</b>\n"
        f"• Команда: <code>{escape(str(lot.get('command')))}</code>\n"
        f"• В плагине: <b>{'ВКЛ' if lot.get('enabled', True) else 'ВЫКЛ'}</b>\n"
        f"• На FunPay: <b>{_lot_funpay_state_text(lot)}</b>\n"
        f"• Последняя проверка FunPay: <b>{escape(sync_text)}</b>\n"
        f"• Привязанный аккаунт: <b>{'1' if bound else '0'}</b>\n"
        f"• Denuvo: <b>{'ВКЛ через аккаунт' if _is_denuvo_lot(lot) else 'ВЫКЛ'}</b>\n\n"
...
def _denuvo_center_text() -> str:
    ...
    bad_codes = {
        "NOT_CONFIGURED", "NETWORK_ERROR", "TIMEOUT", "SERVER_UNAVAILABLE",
        "INSTALLATION_DISABLED_OR_UNKNOWN", "LICENSE_DISABLED", "LICENSE_EXPIRED",
        "BUILD_NOT_ALLOWED", "BUILD_HASH_MISMATCH", "MACHINE_HASH_MISMATCH",
    }
    api_working = configured and registered and last_code not in bad_codes and not last_code.startswith("HTTP_")

    return (
        "<b>🎮 Denuvo</b>\n\n"
        f"• API: <b>{'работает' if api_working else 'не работает'}</b>\n\n"
        f"• В очереди: <b>{queue_count}</b>\n"
        f"• Активные резервации API: <b>{active_reservations}</b>\n"
        f"• Завершено: <b>{completed}</b>\n"
        f"• Ошибки / требуют проверки: <b>{errors}</b>"
    )
...
def _notifications_text() -> str:
    ...
    lines = [
        "<b>🔔 Уведомления</b>",
        "",
        "Нажмите кнопку, чтобы включить или выключить тип уведомлений.",
        "",
    ]
...
def _analytics_text() -> str:
    ...
    lines = [
        ...
        f"• Успешных выдач: <b>{stats.get('issued', 0)}</b>",
        f"• Отказов: <b>{stats.get('denied', 0)}</b>",
        f"• Запросов к серверу: <b>{stats.get('server_requests', 0)}</b>",
        f"• Ошибок сервера: <b>{stats.get('server_errors', 0)}</b>",
        f"• Denuvo поставлено в очередь: <b>{stats.get('queue_enqueued', 0)}</b>",
        f"• Denuvo завершено: <b>{stats.get('queue_completed', 0)}</b>",
        f"• Denuvo ошибок: <b>{stats.get('queue_failed', 0)}</b>",
        "",
        "<b>Выдачи по типам:</b>",
    ]
    if not by_type:
...
def _maintenance_text() -> str:
    ...
    return (
        "<b>🛠 Обслуживание</b>\n\n"
        f"• База: <b>{escape(status)}</b>\n"
        f"• Режим: <code>{escape(mode)}</code>\n"
        f"• Размер encrypted JSON: <b>{size} байт</b>"
    )
...
def _security_text() -> str:
    ...
    return (
        "<b>🗃 Хранение данных</b>\n\n"
        f"• Состояние базы: <b>{escape(status)}</b>\n"
        f"• Режим: <code>{escape(mode)}</code>\n"
        f"• Шифрование: <code>Fernet + локальный ключ</code>\n"
        f"• Размер encrypted JSON: <b>{os.path.getsize(DB_FILE) if os.path.isfile(DB_FILE) else 0} байт</b>\n\n"
...
def _state_text() -> str:
    ...
    return (
        "<b>🧩 Состояние</b>\n\n"
        f"• Плагин: <b>{'ВКЛ' if db['global'].get('plugin_enabled', True) else 'ВЫКЛ'}</b>\n"
        f"• Аккаунты в плагине: <b>{enabled_accounts}/{len(accounts)}</b>\n"
        f"• Лоты в плагине: <b>{plugin_lots}/{len(lots)}</b>\n"
        f"• Лоты на FunPay: <b>{funpay_lots}/{len(lots)}</b>"
        f"{' · проверено ' + str(funpay_known) if funpay_known < len(lots) else ''}\n\n"
...
def _start_add_lot(cardinal: "Cardinal", call) -> None:
    ...
    if not _db_get().get("accounts"):
        ...
        _safe_edit(
            ...
            call.message.id,
            "⚠️ <b>Сначала нужен аккаунт</b>\n\n"
            "Лот наследует лимиты, очередь, сообщение и Denuvo из аккаунта.",
            kb,
        )
        return
    chat_id = call.message.chat.id
...
def _finish_add_lot(bot, chat_id: int, state: dict, account_ids: List[str]) -> None:
    ...
    lot = {
        "id": _uid("l"),
        "funpay_lot_id": str(data.get("funpay_lot_id") or ""),
        "title": str(data.get("title") or data.get("funpay_lot_id") or "Лот")[:150],
        "account_type": _canonical_account_type(data.get("account_type")),
        "command": str(data.get("command") or ""),
        "enabled": True,
        "funpay_active": None,
        "funpay_sync": {},
        "account_ids": [selected] if selected else [],
    }
    if not selected:
...
def _show_lot_account_picker(cardinal: "Cardinal", call, mode: str, page: int = 0, lot: Optional[dict] = None) -> None:
    ...
    text = (
        ...
        f"Тип: <b>{escape(ACCOUNT_TYPES.get(_canonical_account_type(account_type), str(account_type)))}</b>\n"
...
def _parse_duration_input(value: str) -> int:
    ...
    match = re.fullmatch(r"(\d{1,9})(с|сек|секунд|s|м|мин|минут|m|ч|час|часов|h|д|дн|дней|d)?", text)
    if not match:
    ...
    if suffix in {"м", "мин", "минут", "m"}:
        ...
        multiplier = 86400
    return min(31_536_000, amount * multiplier)
...
def _account_queue_choice_kb() -> K:
    kb = K()
    kb.row(
        B("✅ Да, включить", callback_data=f"{CB_ACCOUNT_QUEUE_CHOICE}:y"),
        B("❌ Нет", callback_data=f"{CB_ACCOUNT_QUEUE_CHOICE}:n"),
    )
...
def _prompt_account_queue_choice(bot, chat_id: int, state: dict) -> None:
    ...
    if account_type == "totp":
        title = "Очередь TOTP"
        description = (
            "Один TOTP-код действует 30 секунд. Первый покупатель получит текущий код, "
            "а одновременные запросы выстроятся по порядку и получат коды в следующих временных окнах. "
            "Так один и тот же код не уйдёт двум людям."
        )
    else:
        title = "Очередь Steam Guard"
        description = (
            "Если два или больше покупателей отправят команду почти одновременно, первый запрос обработается сразу, "
            "а остальные выстроятся по порядку. Между выдачами одного Steam-аккаунта будет минимум 15 секунд."
        )
    _account_flow_edit(
        bot,
        chat_id,
...
def _account_denuvo_choice_kb(target: str = "add", account_id: str = "") -> K:
    ...
    kb = K()
    kb.row(
        B("✅ Да", callback_data=f"{CB_ACCOUNT_DENUVO_CHOICE}{suffix}:y"),
        B("❌ Нет", callback_data=f"{CB_ACCOUNT_DENUVO_CHOICE}{suffix}:n"),
    )
...
def _prompt_account_denuvo_choice(bot, chat_id: int, state: dict) -> None:
    ...
    _account_flow_edit(
        ...
        chat_id,
        state,
        "🎮 <b>Использовать Denuvo для этого Steam-аккаунта?</b>\n\n"
...
def _finish_add_account(bot, chat_id: int, state: dict) -> None:
    ...
    denuvo_cfg = {
        "enabled": denuvo_enabled,
        "slot_limit": 5,
        "slot_limit_custom": False,
        "reserve": 0,
        "active_slots": 0,
        "weight": 100,
        "failure_rate": 0.0,
    ...
    account = {
        ...
        "queue": {
            ...
            "interval_seconds": TOTP_QUEUE_INTERVAL_SECONDS if account_type == "totp" else STEAM_QUEUE_INTERVAL_SECONDS,
            "last_issue_at": 0,
        },
        "denuvo": denuvo_cfg,
    }
...
def _start_edit_account_denuvo(cardinal: "Cardinal", call) -> None:
    ...
    text = (
        "🎮 <b>Настройки Denuvo</b>\n\n"
        f"• Состояние: <b>{'ВКЛ' if denuvo.get('enabled') else 'ВЫКЛ'}</b>\n"
        f"• Игра: <b>{escape(game)}</b>\n"
        f"• Активации: <b>{active}/{limit}</b>\n\n"
...
def _start_edit_account_secret(cardinal: "Cardinal", call) -> None:
    ...
    if account_type == "steam_sda":
        prompt = (
            ...
            "Отправьте shared_secret из Steam Desktop Authenticator или файл <code>.maFile</code>. "
            "Ссылки <code>otpauth://</code> и TOTP-secret не принимаются.\n\n"
            "Сообщение удалится автоматически."
        )
    elif account_type == "totp":
        prompt = (
            "🔐 <b>Замена TOTP secret</b>\n\n"
        ...
        if mode == "add_account":
            ...
            if step == "secret":
                ...
                if document is not None:
                    ...
                    if account_type == "steam_sda":
                        ...
                        if not generate_totp_code(extracted):
                            raise ValueError(
                                "Невалидный TOTP secret. Нужен Base32 или ссылка otpauth:// с параметром secret."
                            )
                ...
                if inferred_name:
                    ...
                    _account_flow_edit(
                        ...
                        chat_id,
                        state,
                        f"🏷 <b>Название аккаунта</b>\n\n"
                        f"Отправьте понятное название, например <code>{example}</code>.",
                    )
                return
            ...
            if step == "limit_count":
                ...
                _account_flow_edit(
                    ...
                    state,
                    "♻️ <b>Сброс лимита</b>\n\n"
                    f"Лимит: <b>{limit} кодов</b>. Через какое время он должен начинаться заново?\n\n"
                    "Примеры: <code>30с</code>, <code>10м</code>, <code>2ч</code>, <code>1д</code>.\n"
        ...
        if mode == "add_lot":
            if step == "funpay_lot_id":
                ...
                _safe_edit(
                    ...
                    chat_id,
                    panel_msg_id,
                    "🏷 <b>Название лота</b>\n\nОтправьте название игры или товара.",
                    _cancel_kb(CB_LOTS),
                )
                return
            ...
            if step == "title":
                ...
                kb = K()
                available_types = {
                    _canonical_account_type(x.get("type"))
                    for x in _db_get().get("accounts", [])
                    if _canonical_account_type(x.get("type")) in LOT_CREATION_TYPES
                }
                ...
                _safe_edit(
                    ...
                    panel_msg_id,
                    "➕ <b>Добавление лота</b>\n\n"
                    "Выберите тип аккаунта. Все лимиты, очередь, сообщение и Denuvo будут унаследованы из него.",
                    kb,
                )
                return
            ...
            if mode == "plugin_update_local":
                ...
                if result.get("ok") and result.get("changed"):
                    result_text = (
                        ...
                        f"Файл: <code>{escape(str(filename))}</code>\n"
                        f"Версия: <code>{escape(str(result.get('current_version')))}</code> → <code>{escape(str(result.get('remote_version')))}</code>\n"
    ...
    try:
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": f"{NAME}/{VERSION} self-updater",
...
def _plugin_update_online(cardinal: "Cardinal", call) -> None:
    ...
    if result.get("ok") and result.get("changed"):
        ...
        text = (
            "🆕 <b>Найдена новая версия плагина.</b>\n\n"
            f"Текущая версия: <code>{escape(str(result.get('current_version')))}</code>\n"
            f"Новая версия: <code>{escape(str(result.get('remote_version')))}</code>\n\n"
        ...
        text = (
            ...
            f"Установлена версия: <code>{escape(str(result.get('current_version')))}</code>\n"
            f"Версия по ссылке: <code>{escape(str(result.get('remote_version') or 'не определена'))}</code>\n\n"
...
def _logs_html(entries: List[dict]) -> bytes:
    ...
    for item in entries:
        ...
        cards.append(
            f'<article class="log {_log_html_class(event_type)}">'
            f'<div class="head"><span class="badge">{icon} {escape(label)}</span>'
            f'<time>{escape(_fmt_dt(item.get("ts")))}</time></div>'
            f'<div class="message">{escape(str(item.get("message") or ""))}</div>'
            f'<div class="details">{details_html}</div></article>'
        )
...
.log{{background:var(--card);border:1px solid var(--line);border-left:5px solid var(--info);border-radius:12px;padding:14px 16px;margin:10px 0;box-shadow:0 8px 22px #0004}}
...
def init_cardinal(cardinal: "Cardinal") -> None:
    ...
    _log(
        ...
        "Плагин запущен",
        version=VERSION,
        database="заблокирована" if _db_locked() else "готова",
        denuvo_worker=bool(_DENUVO_WORKER is not None and _DENUVO_WORKER.is_alive()),
        code_queue_worker=bool(_STEAM_QUEUE_WORKER is not None and _STEAM_QUEUE_WORKER.is_alive()),
        build_hash=_plugin_build_hash(),
    )
